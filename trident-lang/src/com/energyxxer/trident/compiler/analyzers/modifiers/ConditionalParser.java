    ...
    public ExecuteModifier parseSingle(TokenPattern<?> pattern, ISymbolContext ctx) {
        ...
        switch(subject.getName()) {
            case "PREDICATE_CONDITION": {
                TridentUtil.ResourceLocation loc = CommonParsers.parseResourceLocation(subject.find("RESOURCE_LOCATION"), ctx);
            ...
            case "SCORE_CONDITION": {
                LocalScore scoreA = CommonParsers.parseScore(subject.find("SCORE"), ctx);
                TokenStructure choice = (TokenStructure) subject.find("CHOICE");
                String branchName = choice.getContents().getName();
                try {
                    switch(branchName) {
                        case "COMPARISON": {
                            LocalScore scoreB = CommonParsers.parseScore(choice.find("SCORE"), ctx);
