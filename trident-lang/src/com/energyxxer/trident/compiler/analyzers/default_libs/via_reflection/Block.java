...
public class Block {
    public static boolean exists(ResourceLocation loc, ISymbolContext ctx) {
        return ctx.get(SetupModuleTask.INSTANCE).namespaceExists(loc.namespace) && ctx.get(SetupModuleTask.INSTANCE).getNamespace(loc.namespace).types.block.exists(loc.body);
    }
